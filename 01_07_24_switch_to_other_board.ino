...
void setup()
{
  ...
  Serial1.begin(9600);  //changed myserial
  Serial1.write(0xE1); // works without?  changed myserial
  Serial1.write(0xA1); //changed myserial
...
{
  if (Serial1.available() > 0)  // changed myserial
  {
    incomingByte = Serial1.read(); //changed myserial
      switch (state) 
      {
        case 1:
